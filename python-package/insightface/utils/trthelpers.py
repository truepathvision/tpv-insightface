...
class HostDeviceMem:
    def __init__(self, size: int, dtype: Optional[np.dtype] = None):
        ...
        self._host_ptr = cuda_call(cudart.cudaMallocHost(self._nbytes))
        pointer_type = ctypes.POINTER(np.ctypeslib.as_ctypes_type(dtype))
        self._host = np.ctypeslib.as_array(ctypes.cast(self._host_ptr, pointer_type), (size,))

        # Allocate device memory
        self._device = cuda_call(cudart.cudaMalloc(self._nbytes))
    ...
    def host(self, data: Union[np.ndarray, bytes]):
        if isinstance(data, np.ndarray):
            if data.size > self.host.size:
                raise ValueError("Too much data for host buffer")
            np.copyto(self.host[:data.size], data.flat, casting='safe')
        else:
            assert self.host.dtype == np.uint8
            self.host[:self._nbytes] = np.frombuffer(data, dtype=np.uint8)
    ...
    def free(self):
        ...
        try:
            if self._device:
                cuda_call(cudart.cudaFree(self._device))
                self._device = None
        except Exception as e:
            print(f"[WARN] cudaFree failed: {e}")
        ...
        try:
            ...
            print(f"[WARN] cudaFreeHost failed: {e}") 
